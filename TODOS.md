...
- [ ] **DEBT-F4b** — audit `Event.tenant_id` must be populated from caller's tenant. Test: `debt_f04b_audit_event_tenant_must_be_populated` @ same file.
...
- [ ] **DEBT-F5** — `schema_register` / `schema_alter` must not hardcode actor `"system"`. Test: `debt_f05_schema_ops_audit_actor_must_not_be_literal_system` @ `shroudb-sigil-engine/src/engine.rs`.
- [ ] **DEBT-F6** — `envelope_lookup` must not pass empty principal to `check_policy`. Test: `debt_f06_lookup_policy_principal_must_not_be_empty` @ same file.
