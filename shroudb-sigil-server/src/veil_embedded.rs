...
impl<S: Store + 'static> VeilOps for EmbeddedVeilOps<S> {
    fn put(&self, entry_id: &str, data: &[u8], field: Option<&str>, blind: bool) -> BoxFut<'_, ()> {
        ...
        Box::pin(async move {
            self.engine
                .put(&self.index, &id, &wire_data, field.as_deref(), blind)
                .await
                .map(|_| ())
        ...
        Box::pin(async move {
            self.engine
                .search(
                    &self.index,
                    &q,
                    shroudb_veil_core::matching::MatchMode::Exact,
                    f.as_deref(),
                    limit,
                    blind,
                )
...
mod tests {
    ...
    async fn put_and_search_round_trips_entry() {
        ...
        ops.put("user-1", entry, Some("email"), false)
            .await
            .expect("put");

        let hits = ops
            .search("alice@example.com", Some("email"), None, false)
            .await
            .expect("search");
        assert!(
    ...
    async fn delete_removes_entry_from_search_hits() {
        let ops = build_ops().await;
        ops.put("user-1", b"bob@example.com", Some("email"), false)
            .await
            .unwrap();
        ops.delete("user-1").await.expect("delete");

        let hits = ops
            .search("bob@example.com", Some("email"), None, false)
            .await
            .unwrap();
        assert!(
            !hits.iter().any(|(id, _)| id == "user-1"),
